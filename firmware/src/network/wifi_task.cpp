...
void WifiTask::webHandlerDemoCommand()
{
    cJSON *root = cJSON_Parse(server_->arg("plain").c_str());
    if (root == nullptr)
    {
    ...
    if (!cJSON_IsString(dev) || dev->valuestring == nullptr || strlen(dev->valuestring) == 0 || !cJSON_IsNumber(val))
    {
        cJSON_Delete(root);
        server_->send(400, "text/plain", "Invalid payload, expected dev:string and val:number");
        return;
    }

    WiFiEvent event;
    event.type = SK_DEMO_COMMAND;
    event.body.demo_command.id = cJSON_IsNumber(id) ? static_cast<uint32_t>(id->valuedouble) : 0;
    strlcpy(event.body.demo_command.dev, dev->valuestring, sizeof(event.body.demo_command.dev));
    event.body.demo_command.val = val->valueint;
    ...
    char response[128];
    snprintf(response, sizeof(response), "{\"queued\":true,\"id\":%u}", event.body.demo_command.id);
    server_->send(200, "application/json", response);
}
...
void WifiTask::startWebServer()
{
    ...
    server_->on("/demo", HTTP_POST, [this]()
                { this->webHandlerDemoCommand(); });
