...
EntityStateUpdate BlindsApp::updateStateFromKnob(PB_SmartKnobState state)
{
    ...
    bool position_changed = last_closed_position != current_closed_position;
    uint8_t position_pct = (20 - current_closed_position) * 5;
    if (force_publish_ && !position_changed)
    {
        position_pct = forced_position_pct_;
    }
    ...
    {
        {
            ...
            {
                lv_label_set_text_fmt(percentage_label, "%d%%", position_pct);
            }
            lv_obj_align(percentage_label, LV_ALIGN_CENTER, 0, 0);
        }
        ...
        cJSON_AddNumberToObject(json, "position", position_pct);
...
int8_t BlindsApp::navigationNext()
{
    ...
    motor_config.position_nonce = motor_config.position;
    force_publish_ = true;
    forced_position_pct_ = (20 - motor_config.position) * 5;
