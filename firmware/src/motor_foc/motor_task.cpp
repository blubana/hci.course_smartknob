...
static void scanI2CDevices()
{
    ...
    for (uint8_t address = 0x08; address < 0x78; address++)
    {
        ...
        if (error == 0)
        {
            LOGI("I2C device found at 0x%02X", address);
            found_any = true;
        }
    }
    if (!found_any)
...
void MotorTask::run()
{
    ...
    {
        LOGI("Auto calibration enabled, starting...");
        calibrate();
        c = configuration_.get();
        LOGI("Auto calibration complete. calibrated=%d", c.motor.calibrated ? 1 : 0);
    }
#else
    ...
    motor.initFOC();
    LOGI("FOC init done. calibrated=%d", c.motor.calibrated ? 1 : 0);
    ...
    while (1)
    {
        ...
        if (xQueueReceive(queue_, &command, 0) == pdTRUE)
        {
            ...
            case CommandType::CONFIG:
            {
                ...
                const float detent_strength_scale = 0.5f;
                const float derivative_lower_strength = config.detent_strength_unit * 0.08f * detent_strength_scale;
                const float derivative_upper_strength = config.detent_strength_unit * 0.02f * detent_strength_scale;
        ...
        const float detent_strength_scale = 0.5f;
        motor.PID_velocity.P = out_of_bounds ? config.endstop_strength_unit * 4.0f * detent_strength_scale : config.detent_strength_unit * 4.0f * detent_strength_scale;
...
#elif SENSOR_AS5600
    // No explicit error reporting for AS5600 in this driver.
#endif
}
