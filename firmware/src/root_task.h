...
#include "proto/demo_uart_protocol.h"
...
public:
    RootTask(const uint8_t task_core, Configuration *configuration, MotorTask &motor_task, DisplayTask *display_task, WifiTask *wifi_task, MqttTask *mqtt_task, SpotifyTask *spotify_task, LedRingTask *led_ring_task, SensorsTask *sensors_task, ResetTask *reset_task, FreeRTOSAdapter *free_rtos_adapter, SerialProtocolPlaintext *serial_protocol_plaintext, SerialProtocolProtobuf *serial_protocol_protobuf, DemoUartProtocol *demo_uart_protocol);
    virtual ~RootTask();
...
private:
    ...
    DemoUartProtocol *demo_uart_protocol_;
    ...
    uint8_t last_button_pressed_played_ = VIRTUAL_BUTTON_IDLE;
