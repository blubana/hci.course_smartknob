...
RootTask::RootTask(
    ...
    ResetTask *reset_task, FreeRTOSAdapter *free_rtos_adapter, SerialProtocolPlaintext *serial_protocol_plaintext, SerialProtocolProtobuf *serial_protocol_protobuf, DemoUartProtocol *demo_uart_protocol) : Task("RootTask", 1024 * 18, ESP_TASK_MAIN_PRIO, task_core),
                                                                                                                                                                       ...
                                                                                                                                                                       demo_uart_protocol_(demo_uart_protocol)
{
#if SK_DISPLAY
    ...
    while (1)
    {
        ...
        if (xQueueReceive(shared_events_queue, &wifi_event, 0) == pdTRUE)
        {
            ...
            case SK_SPOTIFY_VOLUME:
                ...
                break;
            case SK_DEMO_COMMAND:
                if (demo_uart_protocol_ != nullptr && configuration_->getOSConfiguration()->mode == OSMode::DEMO)
                {
                    demo_uart_protocol_->sendCommandWithId(
                        wifi_event.body.demo_command.id,
                        wifi_event.body.demo_command.dev,
                        wifi_event.body.demo_command.val);
                }
                break;
        ...
        if (xQueueReceive(knob_state_queue_, &latest_state_, 0) == pdTRUE)
        {
            ...
            if (demo_uart_protocol_ != nullptr && app_state.os_mode_state == OSMode::DEMO)
            {
                demo_uart_protocol_->sendFromEntityState(entity_state_update_to_send);
            }
        ...
        if (demo_uart_protocol_ != nullptr)
        {
            demo_uart_protocol_->loop();
        }
...
void RootTask::updateHardware(AppState *app_state)
{
    ...
    auto dispatch_navigation_event = [&](NavigationEvent event) {
    ...
    if (configuration_loaded_)
    {
        ...
        case VIRTUAL_BUTTON_LONG_PRESSED:
            if (last_strain_pressed_played_ != VIRTUAL_BUTTON_LONG_PRESSED)
            {
                ...
                dispatch_navigation_event(event);
            }
            break;
    ...
    if (configuration_loaded_)
    {
        switch (latest_sensors_state_.button.virtual_button_code)
        {
        case VIRTUAL_BUTTON_SHORT_PRESSED:
            if (last_button_pressed_played_ != VIRTUAL_BUTTON_SHORT_PRESSED)
            {
                LOGI("Nav button pressed");
                engage_screen();
                motor_task_.playHaptic(true, false);
                last_button_pressed_played_ = VIRTUAL_BUTTON_SHORT_PRESSED;
            }
            break;
        case VIRTUAL_BUTTON_LONG_PRESSED:
            if (last_button_pressed_played_ != VIRTUAL_BUTTON_LONG_PRESSED)
            {
                LOGI("Nav button long pressed");
                engage_screen();
                motor_task_.playHaptic(true, true);
                last_button_pressed_played_ = VIRTUAL_BUTTON_LONG_PRESSED;
                NavigationEvent event = NavigationEvent::LONG;
                dispatch_navigation_event(event);
            }
            break;
        case VIRTUAL_BUTTON_SHORT_RELEASED:
            if (last_button_pressed_played_ != VIRTUAL_BUTTON_SHORT_RELEASED)
            {
                LOGI("Nav button released");
                motor_task_.playHaptic(false, false);
                last_button_pressed_played_ = VIRTUAL_BUTTON_SHORT_RELEASED;
                NavigationEvent event = NavigationEvent::SHORT;
                dispatch_navigation_event(event);
            }
            break;
        case VIRTUAL_BUTTON_LONG_RELEASED:
            if (last_button_pressed_played_ != VIRTUAL_BUTTON_LONG_RELEASED)
            {
                LOGI("Nav button long released");
                motor_task_.playHaptic(false, false);
                last_button_pressed_played_ = VIRTUAL_BUTTON_LONG_RELEASED;
            }
            break;
        default:
            last_button_pressed_played_ = VIRTUAL_BUTTON_IDLE;
            break;
        }
    }
#endif
