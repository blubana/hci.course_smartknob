...
bool Configuration::loadWiFiConfiguration()
{
    ...
    if (!is_wifi_set && LOCAL_WIFI_SSID[0] != '\0')
    {
        snprintf(wifi_config.ssid, sizeof(wifi_config.ssid), "%s", LOCAL_WIFI_SSID);
        snprintf(wifi_config.passphrase, sizeof(wifi_config.passphrase), "%s", LOCAL_WIFI_PASSWORD);
        is_wifi_set = true;
        LOGI("Using WiFi credentials from local_secrets.h");
    }
#endif
